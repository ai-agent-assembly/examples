"""Real Strands Agents tools for the strands-agents-tool-policy example.

Each tool is a genuine ``@strands.tool`` (a ``DecoratedFunctionTool``) — the same
object a Strands ``Agent`` invokes when the model chooses a tool. Three tools:
