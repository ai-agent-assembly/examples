#!/usr/bin/env python3
"""Enforce ADR 0033's claim vocabulary and banned-absolutes rules against this
repo's reader-facing Markdown. AAASM-5798 / AAASM-5599.

Adapted from agent-assembly's `scripts/check_claim_vocabulary.py` (AAASM-5679),
via docs Hub's, python-sdk's, node-sdk's, go-sdk's and arena's (AAASM-5797/5798)
ports. Per AAASM-5795's federation-by-convention decision this is a ONE-TIME
PORT, not a shared dependency -- this file diverges independently from here.
This is the fifth and final AAASM-5798 repo-slice.

File scope: `docs/**`, `README.md` (root and every per-example
`<lang>/<example>/README.md` -- this repo's whole tree is READMEs, per its own
CLAUDE.md: "no shared build at the root", one small self-contained project per
leaf directory), `CONTRIBUTING.md`. Excludes `verification-reports/**` (past
QA verification write-ups, not forward-facing product claims -- same carve-out
as every prior port) and `node_modules/**` (defensive; none present today).
`snippets/README.md` (hand-authored, explains the extraction mechanism) stays
in scope; the *generated* snippet files it documents are `.py`/`.ts`/`.go`/
`.json`, not Markdown, so no separate exclusion glob is needed for them.
...
def selftest() -> int:
    ...
    if not in_scope("docs/concepts.md"):
        failures.append("in_scope() rejects a real docs/ page")
    if not in_scope("python/crewai-research-crew/README.md"):
        failures.append("in_scope() rejects a per-example README")
    if not in_scope("snippets/README.md"):
        failures.append("in_scope() rejects the hand-authored snippets README")
    if in_scope("verification-reports/AAASM-2195.md"):
        failures.append("in_scope() accepts a past verification report")
    if in_scope("docs/concepts.txt"):
